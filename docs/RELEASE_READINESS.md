...
| External notifications          | **Launch blocker** | Outbox, retries, channel idempotency, preferences, quiet hours, Brevo with Resend fallback, Meta templates, and Web Push are implemented. Twilio WhatsApp Sandbox may validate phone-code UX in staging only; provision/verify Meta and Web Push production credentials before launch. |
