...
- Every successful Vercel production web deployment now triggers a separate, read-only mobile and
  desktop audit of the canonical public alias; API-only deployments are excluded and failure
...
| Sales and booking lifecycle     | Code-complete      | Claim through successful completion plus independent cancellation and no-show outcomes pass locally in mobile and desktop browsers; repeat the suite against managed staging dependencies.                                                                                                                                                                        |
...
| CI and staging                  | **Launch blocker** | CI migrates/seeds PostgreSQL and runs public, anonymous-boundary, authenticated role, and request-to-completion browser suites with retained failure artifacts. Repeat authenticated acceptance against managed staging dependencies after real providers and approved policies are configured.                                                                   |
