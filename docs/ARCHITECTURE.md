...
New administrator identities use temporary credentials. Before workspace access, the administrator must enroll TOTP MFA, verify an owner-controlled primary email, and replace the temporary password. Sales email identities are fixed and administrator-managed. Sales recovery never reveals an existing password: an administrator issues a new temporary password, active sessions and pending login challenges are revoked, the employee must replace it after MFA, and the action is audit logged with a reason.
